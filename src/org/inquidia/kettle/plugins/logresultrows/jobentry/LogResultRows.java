...
public class LogResultRows extends JobEntryBase implements Cloneable, JobEntryInterface {
    ...
    public LogResultRows(String name) {
        super( name, "");
        limit="0";
        loggingLevel=null;
        setID(-1L);
    ...
    public LogResultRows() {
        this("");
        // clear();
    }

    public Object clone() {
        LogResultRows lrr = (LogResultRows) super.clone();
        return lrr;
    }
    ...
    public void loadXML( Node entrynode, List<DatabaseMeta> databases, List<SlaveServer> slaveServers, Repository rep ) throws KettleXMLException {
        try {
            ...
            throw new KettleXMLException( BaseMessages.getString(PKG, "LogResultRows.Error.Exception.UnableLoadXML"), dbe );
        }
    }
    ...
    public void loadRep( Repository rep, IMetaStore metaStore, ObjectId id_jobentry, List<DatabaseMeta> databases,
        ...
        try {
            ...
            throw new KettleException( BaseMessages.getString( PKG, "LogResultRows.Error.Exception.UnableLoadRep" )
                    + id_jobentry, dbe );
        }
    }
    ...
    public void saveRep( Repository rep, IMetaStore metaStore, ObjectId id_job ) throws KettleException {
        try {
            ...
            throw new KettleDatabaseException( BaseMessages.getString( PKG, "LogResultRows.Error.Exception.UnableSaveRep" )
                    + getObjectId(), dbe );
        }
    }
    ...
    public Result execute( Result previousResult, int nr ) throws KettleException {
        ...
        writeToLog( logLevel, "=========================== Write Result Rows to Log - " + getName() + " =====================");

        Iterator<RowMetaAndData> itRows = rows.iterator();
        int counter=0;
        ...
        while( itRows.hasNext() )
        {
            ...
            RowMetaAndData rmad = itRows.next();
            RowMetaInterface rowMeta = rmad.getRowMeta();
            List<ValueMetaInterface> valueMetaList = rowMeta.getValueMetaList();
            Iterator<ValueMetaInterface> itValueMetaList = valueMetaList.iterator();
            writeToLog( logLevel, "--------------------------- Row "+ counter + " ---------------------------");
            while ( itValueMetaList.hasNext() )
            {
                ValueMetaInterface valueMeta = itValueMetaList.next();
                String fieldname = valueMeta.getName();
                String value = rmad.getString( fieldname, "" );
                writeToLog( logLevel, fieldname+" = "+value );
